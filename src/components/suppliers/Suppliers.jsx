...
const Suppliers = () => {
  ...
  const fetchSuppliers = async (currentPage) => {
    ...
    try {
      const response = await listSuppliers(currentPage - 1, pageSize);
      const data = response.data.content.content;

      console.log(`>>> Check data response of API ListSuppliers: `, data);
  ...
  useEffect(() => {
    fetchSuppliers(page); // Gọi API mỗi khi `page` thay đổi
  }, [page]);
  ...
  if (totalCount === 0) return <div>No suppliers available.</div>;
              ...
              <tr key={supplier.id}>
                ...
                {/* <td className="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                  <div className="flex justify-end space-x-2">
                    <button
                      className="px-5 py-2 border-teal-500 border text-teal-500 rounded transition duration-300 hover:bg-teal-700 hover:text-white focus:outline-none"
                      //onClick={}
                    >
                      Export Feedback
                    </button>
                ...
                <td className="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                  <div className="flex justify-end space-x-2">
                    <button
                      ...
                      onClick={() => handleSupplierDetails(supplier.id)}
                    >
                      View Details
                    </button>
                  </div>
